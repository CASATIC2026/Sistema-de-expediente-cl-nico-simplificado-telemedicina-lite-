            ...
            var citas = await _context.Citas
                ...
                .Select(c => new
                {
                    ...
                    Consulta = c.Consulta == null ? null : new
                    {
                        ...
                        c.Consulta.Fecha,
                        c.Consulta.TieneOrdenLaboratorio,
                        c.Consulta.EstadoOrden,
                        c.Consulta.ObservacionesLaboratorio
                    }
                })
                .ToListAsync();
            ...
            var citas = await query
                ...
                .Select(c => new {
                    ...
                    consulta = c.Consulta == null ? null : new {
                        ...
                        sintomas                 = c.Consulta.Sintomas,
                        tratamiento              = c.Consulta.Tratamiento,
                        observaciones            = c.Consulta.Observaciones,
                        medicamentosJson         = c.Consulta.MedicamentosJson,
                        tieneOrdenLaboratorio    = c.Consulta.TieneOrdenLaboratorio,
                        estadoOrden              = c.Consulta.EstadoOrden,
                        observacionesLaboratorio = c.Consulta.ObservacionesLaboratorio,
                        resultadoPdfPath         = c.Consulta.ResultadoPdfPath
                    }
                })
                .ToListAsync();
       ...
       // =========================================================
            ...
            [FromQuery] string fecha)
        {
            ...
            while (cursor.Add(TimeSpan.FromMinutes(duracion)) <= horario.HoraFin)
            {
                ...
                var inicio = fechaDate.Date.Add(cursor);
        ...
        public async Task<IActionResult> IniciarConsulta(int id)
        {
            ...
            cita.FechaInicioConsulta = DateTimeOffset.UtcNow;
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> FinalizarConsulta(int id)
        {
            ...
            var existeConsulta = await _context.Consultas
                .AnyAsync(c => c.CitaId == id);
                    ...
                    if (!existeConsulta)
                    {
                        ...
                        {
                            message = "Debes registrar la consulta médica antes de finalizar."
                        });
                    }
           ...
           if (
            ...
            )
            {
                return BadRequest(new
                {
                    message = "No puedes finalizar esta cita."
                });
            }

            cita.Estado = CitaEstados.Finalizada;
            cita.FechaFinConsulta = DateTimeOffset.UtcNow;
            await _context.SaveChangesAsync();
        ...
        // =========================================================
        // MIS ÓRDENES DE LABORATORIO (Paciente)
        // =========================================================
        [Authorize(Roles = Roles.Paciente)]
        [HttpGet("mis-ordenes-laboratorio")]
        public async Task<IActionResult> GetMisOrdenesLaboratorio()
        {
            var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (userIdClaim == null) return Unauthorized();
            var userId = int.Parse(userIdClaim);

            var ordenes = await _context.Citas
                .Include(c => c.Doctor)
                .Include(c => c.Consulta)
                .Where(c =>
                    c.PacienteId == userId &&
                ...
                {
                    citaId            = c.IdCita,
                    consultaId        = c.Consulta!.IdConsulta,
                    fecha             = c.FechaInicio,
                    tipoConsulta      = c.TipoConsulta,
                    doctor            = c.Doctor != null
                                        ? c.Doctor.Nombre + " " + c.Doctor.Apellido
        ...
        // =========================================================
        // Para marcar citas como no asistidas
        [Authorize(Roles = Roles.Doctor)]
        [HttpPut("{id}/no-asistida")]
        public async Task<IActionResult> MarcarNoAsistida(int id)
        {
            var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (userIdClaim == null) return Unauthorized();
            var userId = int.Parse(userIdClaim);
            ...
            if (cita.Estado != CitaEstados.EnConsulta)
                return BadRequest(new { message = "Solo puedes marcar como no asistida una cita en curso." });

            cita.Estado      = CitaEstados.NoAsistida;
            cita.LinkReunion = null;
            await _context.SaveChangesAsync();

            return Ok(new { message = "Cita marcada como no asistida." });
        }


        // =========================================================
        // RESUMEN ESTADÍSTICO DEL PACIENTE (Solo Admin)
        [Authorize(Roles = Roles.Admin)]
        [HttpGet("resumen-paciente/{pacienteId}")]
        public async Task<IActionResult> GetResumenPaciente(int pacienteId)
        {
            var citas = await _context.Citas
                .Include(c => c.Doctor)
                .Where(c => c.PacienteId == pacienteId)
            ...
            if (!citas.Any())
                return Ok(new {
                    ...
                    doctorUltimaConsulta = (string?)null
                });

            var ultimaCita = citas
                .Where(c => c.Estado == CitaEstados.Finalizada)
                .OrderByDescending(c => c.FechaInicio)
                .FirstOrDefault();

            return Ok(new {
                total                = citas.Count,
                finalizadas          = citas.Count(c => c.Estado == CitaEstados.Finalizada),
                canceladas           = citas.Count(c => c.Estado == CitaEstados.Cancelada),
                noAsistidas          = citas.Count(c => c.Estado == CitaEstados.NoAsistida),
                fechaUltimaConsulta  = ultimaCita?.FechaInicio.ToString("dd/MM/yyyy"),
                doctorUltimaConsulta = ultimaCita?.Doctor != null
                    ? ultimaCita.Doctor.Nombre + " " + ultimaCita.Doctor.Apellido
                    : "Sin asignar"
            });
        }
