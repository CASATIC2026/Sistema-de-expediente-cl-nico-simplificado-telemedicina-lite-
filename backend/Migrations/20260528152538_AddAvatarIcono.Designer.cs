// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using TelMedAPI.Data;

#nullable disable

namespace TelMedAPI.Migrations
{
    [DbContext(typeof(TelMedAPIContext))]
    [Migration("20260528152538_AddAvatarIcono")]
    partial class AddAvatarIcono
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("TelMedAPI.Models.Cita", b =>
                {
                    b.Property<int>("IdCita")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id_cita");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdCita"));

                    b.Property<int?>("DoctorId")
                        .HasColumnType("integer")
                        .HasColumnName("doctor_id");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("estado");

                    b.Property<DateTimeOffset>("FechaFin")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_fin");

                    b.Property<DateTimeOffset?>("FechaFinConsulta")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_fin_consulta");

                    b.Property<DateTimeOffset>("FechaInicio")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_inicio");

                    b.Property<DateTimeOffset?>("FechaInicioConsulta")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_inicio_consulta");

                    b.Property<string>("LinkReunion")
                        .HasColumnType("text")
                        .HasColumnName("link_reunion");

                    b.Property<string>("Motivo")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("motivo");

                    b.Property<bool>("NotificacionRecordatorioEnviada")
                        .HasColumnType("boolean")
                        .HasColumnName("notificacion_recordatorio_enviada");

                    b.Property<int>("PacienteId")
                        .HasColumnType("integer")
                        .HasColumnName("paciente_id");

                    b.Property<string>("TipoConsulta")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("tipo_consulta");

                    b.HasKey("IdCita");

                    b.HasIndex("DoctorId");

                    b.HasIndex("PacienteId");

                    b.ToTable("cita");
                });

            modelBuilder.Entity("TelMedAPI.Models.ClinicProfile", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("ClinicName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Horario")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Slogan")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Telefono")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("ClinicProfiles");
                });

            modelBuilder.Entity("TelMedAPI.Models.Consulta", b =>
                {
                    b.Property<int>("IdConsulta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id_consulta");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdConsulta"));

                    b.Property<int>("CitaId")
                        .HasColumnType("integer")
                        .HasColumnName("cita_id");

                    b.Property<string>("Diagnostico")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("diagnostico");

                    b.Property<int?>("DiasIncapacidad")
                        .HasColumnType("integer")
                        .HasColumnName("dias_incapacidad");

                    b.Property<string>("EstadoOrden")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("estado_orden");

                    b.Property<string>("Evolucion")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("evolucion");

                    b.Property<string>("ExamenesJson")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("examenes_json");

                    b.Property<DateTime>("Fecha")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha");

                    b.Property<DateTime?>("FechaFinIncapacidad")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_fin_incapacidad");

                    b.Property<DateTime?>("FechaInicioIncapacidad")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_inicio_incapacidad");

                    b.Property<string>("MedicamentosJson")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("medicamentos_json");

                    b.Property<string>("MotivoIncapacidad")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("motivo_incapacidad");

                    b.Property<string>("Observaciones")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("observaciones");

                    b.Property<string>("ObservacionesIncapacidad")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("observaciones_incapacidad");

                    b.Property<string>("ObservacionesLaboratorio")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("observaciones_laboratorio");

                    b.Property<string>("ResultadoPdfPath")
                        .HasColumnType("text")
                        .HasColumnName("resultado_pdf_path");

                    b.Property<string>("Sintomas")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("sintomas");

                    b.Property<bool>("TieneIncapacidad")
                        .HasColumnType("boolean")
                        .HasColumnName("tiene_incapacidad");

                    b.Property<bool>("TieneOrdenLaboratorio")
                        .HasColumnType("boolean")
                        .HasColumnName("tiene_orden_laboratorio");

                    b.Property<string>("Tratamiento")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("tratamiento");

                    b.HasKey("IdConsulta");

                    b.HasIndex("CitaId")
                        .IsUnique();

                    b.ToTable("consulta");
                });

            modelBuilder.Entity("TelMedAPI.Models.HorarioDoctor", b =>
                {
                    b.Property<int>("IdHorario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id_horario");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("IdHorario"));

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<int>("DiaSemana")
                        .HasColumnType("integer")
                        .HasColumnName("dia_semana");

                    b.Property<int>("DoctorId")
                        .HasColumnType("integer")
                        .HasColumnName("doctor_id");

                    b.Property<TimeSpan>("HoraFin")
                        .HasColumnType("interval")
                        .HasColumnName("hora_fin");

                    b.Property<TimeSpan>("HoraInicio")
                        .HasColumnType("interval")
                        .HasColumnName("hora_inicio");

                    b.HasKey("IdHorario");

                    b.HasIndex("DoctorId");

                    b.ToTable("horario_doctor");

                    b.HasData(
                        new
                        {
                            IdHorario = 1,
                            Activo = true,
                            DiaSemana = 1,
                            DoctorId = 0,
                            HoraFin = new TimeSpan(0, 16, 0, 0, 0),
                            HoraInicio = new TimeSpan(0, 8, 0, 0, 0)
                        },
                        new
                        {
                            IdHorario = 2,
                            Activo = true,
                            DiaSemana = 2,
                            DoctorId = 0,
                            HoraFin = new TimeSpan(0, 16, 0, 0, 0),
                            HoraInicio = new TimeSpan(0, 8, 0, 0, 0)
                        },
                        new
                        {
                            IdHorario = 3,
                            Activo = true,
                            DiaSemana = 3,
                            DoctorId = 0,
                            HoraFin = new TimeSpan(0, 16, 0, 0, 0),
                            HoraInicio = new TimeSpan(0, 8, 0, 0, 0)
                        },
                        new
                        {
                            IdHorario = 4,
                            Activo = true,
                            DiaSemana = 4,
                            DoctorId = 0,
                            HoraFin = new TimeSpan(0, 16, 0, 0, 0),
                            HoraInicio = new TimeSpan(0, 8, 0, 0, 0)
                        },
                        new
                        {
                            IdHorario = 5,
                            Activo = true,
                            DiaSemana = 5,
                            DoctorId = 0,
                            HoraFin = new TimeSpan(0, 16, 0, 0, 0),
                            HoraInicio = new TimeSpan(0, 8, 0, 0, 0)
                        },
                        new
                        {
                            IdHorario = 6,
                            Activo = true,
                            DiaSemana = 6,
                            DoctorId = 0,
                            HoraFin = new TimeSpan(0, 12, 0, 0, 0),
                            HoraInicio = new TimeSpan(0, 9, 0, 0, 0)
                        });
                });

            modelBuilder.Entity("TelMedAPI.Models.Usuario", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AvatarIcono")
                        .HasColumnType("text");

                    b.Property<string>("DUI")
                        .HasColumnType("text");

                    b.Property<bool>("DebeCambiarPassword")
                        .HasColumnType("boolean");

                    b.Property<string>("Direccion")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("Direccion");

                    b.Property<bool>("Eliminado")
                        .HasColumnType("boolean");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("EmailVerificationExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("EmailVerificationToken")
                        .HasColumnType("text");

                    b.Property<bool>("EmailVerified")
                        .HasColumnType("boolean");

                    b.Property<string>("Especialidad")
                        .HasColumnType("text");

                    b.Property<DateOnly>("FechaNacimiento")
                        .HasColumnType("date");

                    b.Property<string>("FotoUrl")
                        .HasColumnType("text");

                    b.Property<string>("Genero")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("GoogleId")
                        .HasColumnType("text");

                    b.Property<string>("JVPM")
                        .HasColumnType("text");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("text");

                    b.Property<DateTime?>("PasswordResetExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("PasswordResetToken")
                        .HasColumnType("text");

                    b.Property<string>("Rol")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Telefono")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.ToTable("Usuarios");
                });

            modelBuilder.Entity("TelMedAPI.Models.Cita", b =>
                {
                    b.HasOne("TelMedAPI.Models.Usuario", "Doctor")
                        .WithMany("CitasComoDoctor")
                        .HasForeignKey("DoctorId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("TelMedAPI.Models.Usuario", "Paciente")
                        .WithMany("CitasComoPaciente")
                        .HasForeignKey("PacienteId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Doctor");

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("TelMedAPI.Models.Consulta", b =>
                {
                    b.HasOne("TelMedAPI.Models.Cita", "Cita")
                        .WithOne("Consulta")
                        .HasForeignKey("TelMedAPI.Models.Consulta", "CitaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cita");
                });

            modelBuilder.Entity("TelMedAPI.Models.HorarioDoctor", b =>
                {
                    b.HasOne("TelMedAPI.Models.Usuario", "Doctor")
                        .WithMany()
                        .HasForeignKey("DoctorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Doctor");
                });

            modelBuilder.Entity("TelMedAPI.Models.Cita", b =>
                {
                    b.Navigation("Consulta");
                });

            modelBuilder.Entity("TelMedAPI.Models.Usuario", b =>
                {
                    b.Navigation("CitasComoDoctor");

                    b.Navigation("CitasComoPaciente");
                });
#pragma warning restore 612, 618
        }
    }
}
