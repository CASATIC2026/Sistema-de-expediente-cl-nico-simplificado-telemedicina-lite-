...
{
  mensajeError.value =
    'Si defines una contraseña debe cumplir los requisitos de seguridad.'
  return
}
  ...
  try {
    ...
    // GOOGLE (flujo normal con login)
      ...
      try {
        ...
        mensajeError.value = 'Error al guardar la sesión. Intente de nuevo.'
        return
      }
    ...
    >
      ...
      <div class="flex flex-col gap-5">
        ...
        <div v-if="!esGoogle" class="flex flex-col gap-2">
          ...
          <input v-model="form.email" type="email" required class="bg-white/5 border border-white/10 p-3 rounded-xl outline-none text-white focus:ring-2 focus:ring-cyan-500/50 placeholder:text-slate-500" placeholder="ejemplo@gmail.com"/>
        </div>
        ...
        <div v-if="esGoogle" class="flex flex-col gap-2">
          ...
          <p class="text-xs text-slate-400">
             Puedes iniciar sesión con Google. Si defines una contraseña también podrás ingresar con correo y contraseña.
            </p>
        </div>

        <div class="flex flex-col gap-2">
          <label class="text-[11px] font-bold uppercase text-slate-400">
            {{ esGoogle ? 'Contraseña (opcional)' : 'Escribe una contraseña *' }}
          </label>
