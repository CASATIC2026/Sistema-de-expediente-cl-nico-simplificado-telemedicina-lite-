...
router.beforeEach((to, from, next) => {
 ...
 // 1. BLOQUEO POR CONSULTA ACTIVA
if (videoStore.isActive) {

  const rutasPermitidas = ['Consultas', 'Consulta']

  // Permitir permanecer dentro del ecosistema de consulta
  if (rutasPermitidas.includes(to.name)) {
    return next()
  }

  // Bloquear navegación hacia otras áreas
  return next(false)
}
