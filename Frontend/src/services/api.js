...
// Perfil de Clinica/farmacia/Negocio
...
export const updateClinicProfile = async (data) => {
  const res = await api.put('/ClinicProfile', data)
  return res.data
}

// ORDEN DE LABORATORIO
export const descargarOrdenLaboratorio = async (consultaId) => {
  const res = await api.get(`/consultas/${consultaId}/orden-pdf`, {
    responseType: 'blob'
  })
  return res.data
}
...
export const getMisOrdenesLaboratorio = async () => {
  const res = await api.get('/citas/mis-ordenes-laboratorio')
  return res.data
}

export const subirResultadoLaboratorio = async (consultaId, archivo) => {
  const formData = new FormData()
  formData.append('archivo', archivo)
  const res = await api.post(`/consultas/${consultaId}/subir-resultado`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' }
  })
  return res.data
}

// REVISAR LABORATORIO (Doctor)
export const revisarLaboratorio = async (consultaId, observaciones) => {
  const res = await api.post(`/consultas/${consultaId}/revisar-laboratorio`, {
    observaciones
  })
  return res.data
}

// DESCARGAR PDF DE RESULTADOS (Doctor)
export const descargarResultadoPdf = async (consultaId) => {
  const res = await api.get(`/consultas/${consultaId}/resultado-pdf`, {
    responseType: 'blob'
  })
  return res.data
}
///MARCAR COMO CITAS NO ASISTIDAS
export const marcarNoAsistida = async (id) => {
  const res = await api.put(`/citas/${id}/no-asistida`)
  return res.data
}
export const getResumenPaciente = async (pacienteId) => {
  const res = await api.get(`/citas/resumen-paciente/${pacienteId}`)
  return res.data
}
